...
// call negotiate
negotiate();
// resize video tag for smaller screens[Temporary HACK]
function resize() {
	var video = document.getElementsByTagName('video');
	var w = window.innerWidth / 1.5;
	var h = window.innerHeight;
	for (i = 0; i < video.length; i++) {
		var ratio = (video[i].clientHeight / video[i].clientWidth);
		if (video[i].clientHeight > h && video[i].clientWidth < w) {
			video[i].style.height = h + "px";
			video[i].style.width = (h / ratio) + "px";
		}
		if (video[i].clientHeight <= h && video[i].clientWidth < w && ratio > 1) {
			video[i].style.height = h + "px";
			video[i].style.width = (h / ratio) + "px";
		}
		if (video[i].clientWidth >= w) {
			video[i].style.width = w + "px";
		}
		if (video[i].clientHeight < h && video[i].clientWidth <= w && ratio < 1) {
			video[i].style.width = w + "px";
		}
